...
var globalConfig = global.opts.plugins && global.opts.plugins.kss ? global.opts.plugins.kss : {};
var config = {
	enabled: true,
...
/*
 * Dynamically render KSS into requested Spec
 *
...
var processRequest = function (req, res, next) {
	...
	if (
		...
		!(req.specData.info.plugins && req.specData.info.plugins.kss && !req.specData.info.plugins.kss.enabled) // Is not disabled per Spec
	) {
		...
		glob(config.targetCssMask, {
			...
			fs.readFile(sectionsTplPath, 'utf-8', function (err, file) {
				if (err) {
					global.log.warn('KSS template not found'+ sectionsTplPath +':', err);
				...
				files.forEach(function (filePath) {
					...
					try {
						...
						kss.parse(file, {}, function (error, parsed) {
							dataForTemplates.sections = dataForTemplates.sections.concat(parsed.data.sections);
						});
					} catch (err) {
						global.log.debug('KSS error parsing '+ fullPath +':', err);
					}
				});
				...
				global.log.debug('KSS processing took:', prettyHrtime(end));
